...
void SendWxMessage()
{
	...
	if (StrStrW(msg->wxid, L"gh"))
	{
		...
		{
			FILE* fg = fopen("gzh.log", "ab+");
			//std::string content = string_To_UTF8(Wchar_tToString(msg->content));
			//fwrite(content.c_str(), content.length(), 1, fg);
			std::string content = string_To_UTF8(Wstring2String(fullmessgaedata));
			fwrite(content.c_str(), content.length(), 1, fg);
			fclose(fg);
			//如果微信ID中带有gh 说明是公众号
