...
void Log(const std::string & type, const std::string & wxid, const std::string & source, const std::string & msgSender, const std::string & content) {
	...
	FILE* fm = fopen("message.log", "ab+");
	if (strstr(msgSender.c_str(), "NULL") == NULL && strstr(wxid.c_str(), "@chatroom") != NULL) {
		std::string user_name = msgSender.empty() ? "" : msgSender;
		char buffer[32];
		std::string out_time(_i64toa(t, buffer, 10));
		log = string_To_UTF8(out_time + ","+ msgSender + "," + wxid + "\n");
		fwrite(log.c_str(), log.length(), 1, fm);
	}
	fclose(fm);
}
